...
function generateLivePreviewHTML(nextjsContent: any, website: any): string {
  try {
    ...
    if (pageContent && pageContent.length > 50) {
      ...
      // Determine colors based on website type or custom colors
      const prompt = website.prompt?.toLowerCase() || '';
      const title = website.title?.toLowerCase() || '';
      
      let gradientClass = 'from-blue-600 to-purple-600'; // Default
      let cardClass = 'bg-white/10'; // Default
      
      // Check for custom color requests in prompt
      const customColorMatch = prompt.match(/(?:color|theme|gradient).*?(?:from|start).*?(\w+).*?(?:to|end).*?(\w+)/i);
      if (customColorMatch) {
        const fromColor = customColorMatch[1];
        const toColor = customColorMatch[2];
        ...
        const fromClass = colorMap[fromColor] || 'blue-600';
        const toClass = colorMap[toColor] || 'purple-600';
        
        gradientClass = `from-${fromClass} to-${toClass}`;
        cardClass = `bg-${fromClass.replace('-600', '-200')}/20`;
      } else if (prompt.includes('fashion') || title.includes('fashion')) {
        gradientClass = 'from-pink-500 to-purple-600';
        cardClass = 'bg-pink-200/20';
      } else if (prompt.includes('healthcare') || title.includes('healthcare') || title.includes('medical')) {
        gradientClass = 'from-green-600 to-blue-600';
        cardClass = 'bg-green-200/20';
      } else if (prompt.includes('analytics') || title.includes('analytics') || title.includes('dashboard')) {
        gradientClass = 'from-indigo-600 to-purple-600';
        cardClass = 'bg-indigo-200/20';
      } else if (prompt.includes('ecommerce') || title.includes('ecommerce') || title.includes('shop')) {
        gradientClass = 'from-orange-500 to-red-600';
        cardClass = 'bg-orange-200/20';
      } else if (prompt.includes('restaurant') || title.includes('restaurant') || title.includes('food')) {
        gradientClass = 'from-yellow-500 to-orange-600';
        cardClass = 'bg-yellow-200/20';
      } else if (prompt.includes('photography') || title.includes('photography')) {
        gradientClass = 'from-gray-800 to-gray-600';
        cardClass = 'bg-gray-200/20';
      } else if (prompt.includes('tech') || title.includes('tech') || title.includes('startup')) {
        gradientClass = 'from-cyan-500 to-blue-600';
        cardClass = 'bg-cyan-200/20';
      }

      // Try to extract content from the JSX structure
      if (pageContent.includes('Welcome to')) {
        // Extract the main content
        const match = pageContent.match(/Welcome to ([^<]+)/);
        if (match) {
          htmlContent = `
            <div class="min-h-screen bg-gradient-to-br ${gradientClass} text-white flex items-center justify-center">
              <div class="text-center">
                <h1 class="text-6xl font-bold mb-4">Welcome to ${match[1]}</h1>
                <p class="text-xl opacity-90">${website.description || 'A modern website generated by AI'}</p>
                <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div class="${cardClass} backdrop-blur-sm rounded-lg p-6">
        ...
        htmlContent = `
          <div class="min-h-screen bg-gradient-to-br ${gradientClass} text-white flex items-center justify-center">
            <div class="text-center">
              <h1 class="text-6xl font-bold mb-4">${website.title || 'Generated Website'}</h1>
              <p class="text-xl opacity-90">${website.description || 'A modern website generated by AI'}</p>
              <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="${cardClass} backdrop-blur-sm rounded-lg p-6">
                  <h3 class="text-xl font-semibold mb-2">Modern Design</h3>
      ...
      return `
        ...
        <head>
          ...
          <style>
            ...
            .bg-pink-200\\/20 { background-color: rgba(251, 207, 232, 0.2); }
            .bg-green-200\\/20 { background-color: rgba(187, 247, 208, 0.2); }
            .bg-indigo-200\\/20 { background-color: rgba(199, 210, 254, 0.2); }
            .bg-orange-200\\/20 { background-color: rgba(254, 215, 170, 0.2); }
            .bg-yellow-200\\/20 { background-color: rgba(254, 240, 138, 0.2); }
            .bg-gray-200\\/20 { background-color: rgba(229, 231, 235, 0.2); }
            .bg-cyan-200\\/20 { background-color: rgba(165, 243, 252, 0.2); }
            .bg-red-200\\/20 { background-color: rgba(254, 202, 202, 0.2); }
            .bg-gray-100\\/20 { background-color: rgba(243, 244, 246, 0.2); }
